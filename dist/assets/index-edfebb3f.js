...
let u, U, y, _, iO = Promise.all([ (() => {
        ...
        for (;;) {
            ...
            if (e == 123) {
                ...
                if (t == 37) {
                    ...
                    for (;;) {
                        let l = O.peek(n);
                        if (l == 32 || l == 10) ++n; else if (l == 35) for (++n; ;) {
                            let r = O.peek(n);
                            if (r < 0 || r == 10) break;
                            n++;
                        } else {
                            if (l != 45 || Q != 2) {
                                let r = l == 101 && O.peek(n + 1) == 110 && O.peek(n + 2) == 100;
                                return void O.acceptToken(r ? 3 : 2, Q);
                            }
                            Q = ++n;
                        }
                    }
        ...
        return new Y(t => {
            ...
            for (;;) {
                ...
                if (l == 123 && t.peek(1) == 37) {
                    ...
                    for (;;r++) {
                        let s = t.peek(r);
                        if (!((Q = s) >= 65 && Q <= 90 || Q >= 97 && Q <= 122)) break;
                ...
                if (t.advance(), l == 10) break;
            }
            var Q;
            t.pos > n && t.acceptToken($);
        });
    }
    const X = v("endcomment", 182, 5), w = v("endraw", 181, 4), S = {
        __proto__: null,
        contains: 32,
        or: 36,
        and: 36,
    ...
    const b = d("abs append at_least at_most capitalize ceil compact concat date default divided_by downcase escape escape_once first floor join last lstrip map minus modulo newline_to_br plus prepend remove remove_first replace replace_first reverse round rstrip size slice sort sort_natural split strip strip_html strip_newlines sum times truncate truncatewords uniq upcase url_decode url_encode where", "function"), T = d("cycle comment endcomment raw endraw echo increment decrement liquid if elsif else endif unless endunless case endcase for endfor tablerow endtablerow break continue assign capture endcapture render include", "keyword"), k = d("empty forloop tablerowloop in with as contains", "keyword"), j = d("first index index0 last length rindex", "property"), Z = d("col col0 col_first col_last first index index0 last length rindex rindex0 row", "property");
    y = function(O = {}) {
        let $ = O.filters ? O.filters.concat(b) : b, e = O.tags ? O.tags.concat(T) : T, t = O.variables ? O.variables.concat(k) : k, {
            properties: n
        } = O;
        return Q => {
            var l;
            let r = function(c) {
                ...
                let {
                    state: g,
                    pos: P
                } = c, a = L(g).resolveInner(P, -1).enterUnfinishedNodesBefore(P), o = ((m = a.childBefore(P)) === null || m === void 0 ? void 0 : m.name) || a.name;
                if (a.name == "FilterName") return {
                    type: "filter",
                    node: a
                };
                if (c.explicit && o == "|") return {
                    type: "filter"
                };
                if (a.name == "TagName") return {
                    type: "tag",
                    node: a
                };
                if (c.explicit && o == "{%") return {
                ...
                if (a.name == "PropertyName" && a.parent.name == "MemberExpression") return {
                    type: "property",
                    node: a,
                    target: a.parent
                };
                if (a.name == "." && a.parent.name == "MemberExpression") return {
                    type: "property",
                    target: a.parent
                };
                if (a.name == "MemberExpression" && o == ".") return {
                    type: "property",
                    target: a
                };
                if (a.name == "VariableName") return {
                    type: "expression",
                    from: a.from
                };
                let q = c.matchBefore(/[\w\u00c0-\uffff]+$/);
                return q ? {
                    type: "expression",
                    from: q.from
                } : c.explicit && a.name != "CommentText" && a.name != "StringLiteral" && a.name != "NumberLiteral" && a.name != "InlineComment" ? {
                    type: "expression"
                } : null;
            }(Q);
            if (!r) return null;
            let p, s = (l = r.from) !== null && l !== void 0 ? l : r.node ? r.node.from : Q.pos;
            return p = r.type == "filter" ? $ : r.type == "tag" ? e : r.type == "expression" ? t : function(c, m, g, P) {
                let a = [];
                for (;;) {
                    let o = m.getChild("Expression");
                    if (!o) return [];
                    if (o.name == "forloop") return a.length ? [] : j;
                    if (o.name == "tablerowloop") return a.length ? [] : Z;
                    if (o.name == "VariableName") {
                        a.unshift(c.sliceDoc(o.from, o.to));
                        break;
                    }
                    if (o.name != "MemberExpression") return [];
                    {
                        let q = o.getChild("PropertyName");
                        q && a.unshift(c.sliceDoc(q.from, q.to)), m = o;
                    }
                }
                return P ? P(a, c, g) : [];
            }(Q.state, r.target, Q, n), p.length ? {
                options: p,
                from: s,
                validFor: /^[\w\u00c0-\uffff]*$/
            } : null;
        };
    }, u = x.inputHandler.of((O, $, e, t) => t == "%" && $ == e && O.state.doc.sliceString($ - 1, e + 1) == "{}" && (O.dispatch(O.state.changeByRange(n => ({
        changes: {
            from: n.from,
            to: n.to,
            insert: "%%"
        },
        range: V.cursor(n.from + 1)
    })), {
        scrollIntoView: !0,
    ...
    const E = D.define({
        ...
        parser: h.configure({
            props: [ F({
                "cycle comment endcomment raw endraw echo increment decrement liquid in with as": i.keyword,
                "empty forloop tablerowloop": i.atom,
                "if elsif else endif unless endunless case endcase for endfor tablerow endtablerow break continue": i.controlKeyword,
                "assign capture endcapture": i.definitionKeyword,
                contains: i.operatorKeyword,
                "render include": i.moduleKeyword,
                VariableName: i.variableName,
                TagName: i.tagName,
                FilterName: i.function(i.variableName),
                ...
                InlineComment: i.lineComment,
                CommentText: i.blockComment,
                "{% %} {{ }}": i.brace,
                "( )": i.paren,
                ".": i.derefOperator,
                ", .. : |": i.punctuation
                ...
                "UnlessDirective ForDirective TablerowDirective CaptureDirective": f(/^\s*(\{%-?\s*)?end\w/),
                IfDirective: f(/^\s*(\{%-?\s*)?(endif|else|elsif)\b/),
                CaseDirective: f(/^\s*(\{%-?\s*)?(endcase|when)\b/)
            }), I.add({
    ...
    _ = R(W.language), U = function(O = {}) {
        let $ = O.base || W, e = $.language == W.language ? _ : R($.language);
        return new A(e, [ $.support, e.data.of({
            autocomplete: y(O)
        }), $.language.data.of({
            closeBrackets: {
                brackets: [ "{" ]
...
export {
    iO as __tla,
    u as closePercentBrace,
    U as liquid,
    y as liquidCompletionSource,
    _ as liquidLanguage
};