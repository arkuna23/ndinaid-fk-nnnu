...
let q, p, z = Promise.all([ (() => {
    ...
    const P = new S(O => {
        ...
        for (;;) {
            ...
            if (O.next == 123 && O.peek(1) == 123 || O.next < 0) break;
            O.advance();
        }
        O.pos > e && O.acceptToken(1);
    });
    function n(O, e, a) {
        return new S(t => {
            let d = t.pos;
            for (;t.next != O && t.next >= 0 && (a || t.next != 38 && (t.next != 123 || t.peek(1) != 123)); ) t.advance();
            t.pos > d && t.acceptToken(e);
        });
    }
    const c = n(39, 33, !1), R = n(34, 34, !1), g = n(39, 35, !0), m = n(34, 36, !0), $ = y.deserialize({
        version: 14,
        ...
        tokenizers: [ P, c, R, g, m, 0, 1 ],
        topRules: {
            Content: [ 0, 2 ],
            Attribute: [ 1, 9 ]
        },
        tokenPrec: 0
    }), v = s.parser.configure({
        ...
        parser: Q.configure({
            wrap: i((O, e) => O.name == "InterpolationContent" ? o : null)
        })
    }, W = {
        parser: Q.configure({
            wrap: i((O, e) => {
                var a;
                return O.name == "InterpolationContent" ? o : O.name != "AttributeInterpolation" ? null : ((a = O.node.parent) === null || a === void 0 ? void 0 : a.name) == "StatementAttributeValue" ? b : o;
            }),
            top: "Attribute"
        })
    }, l = U();
    ...
    function f(O, e) {
        switch (O.name) {
          case "Attribute":
            return /^[*#(\[]|\{\{/.test(e.read(O.from, O.to)) ? W : null;
    ...
    q = function(O = {}) {
        let e = l;
        if (O.base) {
            if (O.base.language.name != "html" || !(O.base.language instanceof T)) throw new RangeError("The base option must be the result of calling html(...)");
            e = O.base;
        }
        return new x(e.language == l.language ? p : u(e.language), [ e.support, e.language.data.of({
            closeBrackets: {
                brackets: [ "[", "{", '"' ]
            },
            indentOnInput: /^\s*[\}\]]$/
