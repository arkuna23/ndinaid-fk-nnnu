...
let s, l, p, W, J = Promise.all([ (() => {
    try {
        return L;
    } catch {}
})() ]).then(async () => {
    const k = new j((Q, e) => {
        for (let i = 0, a = Q.next; (e.context && (a < 0 || a == 10 || a == 13 || a == 47 && Q.peek(i + 1) == 47) || a == 41 || a == 125) && Q.acceptToken(177), 
        a == 32 || a == 9; ) a = Q.peek(++i);
    }, {
        contextual: !0
    });
    let h = new Set([ 95, 184, 20, 12, 17, 144, 145, 142, 148, 13, 53, 25 ]), V, m, Y, R, $, r;
    V = new g({
        start: !1,
        shift: (Q, e) => e == 179 ? Q : h.has(e)
    }), m = U({
        ...
        tokenizers: [ k, 1, 2, new b("j~RQYZXz{^~^O$|~~aP!P!Qd~iO$}~~", 25, 181) ],
        topRules: {
            SourceFile: [ 0, 3 ]
        },
        dynamicPrecedences: {
	...
	\${}
        ...
        detail: "declaration",
        type: "keyword"
    }) ], $ = new I(), r = new Set([ "SourceFile", "Block", "FunctionDecl", "MethodDecl", "FunctionLiteral", "ForStatement", "SwitchStatement", "TypeSwitchStatement", "IfStatement" ]);
    function n(Q, e) {
        return (i, a) => {
            O: for (let t = i.node.firstChild, c = 0, X = null; ;) {
                for (;!t; ) {
                    if (!c) break O;
                    c--, t = X.nextSibling, X = X.parent;
                }
                e && t.name == e || t.name == "SpecList" ? (c++, X = t, t = t.firstChild) : (t.name == "DefName" && a(t, Q), 
                t = t.nextSibling);
            }
            return !0;
        };
    ...
    const d = {
        FunctionDecl: n("function"),
        VarDecl: n("var", "VarSpec"),
        ConstDecl: n("constant", "ConstSpec"),
        TypeDecl: n("type", "TypeSpec"),
        ImportDecl: n("constant", "ImportSpec"),
        Parameter: n("var"),
        __proto__: null
    };
    function x(Q, e) {
        ...
        let a = [], t = !0;
        function c(X, o) {
            let f = Q.sliceString(X.from, X.to);
            a.push({
                label: f,
                type: o
            });
        }
        return e.cursor(N.IncludeAnonymous).iterate(X => {
            if (t) t = !1; else if (X.name) {
                let o = d[X.name];
                if (o && o(X, c) || r.has(X.name)) return !1;
            } else if (X.to - X.from > 8192) {
                for (let o of x(Q, X.node)) a.push(o);
                return !1;
            }
        }), $.set(e, a), a;
    }
    let S, T;
    S = /^[\w$\xa1-\uffff][\w$\d\xa1-\uffff]*$/, T = [ "String", "LineComment", "BlockComment", "DefName", "LabelName", "FieldName", ".", "?." ], 
    p = Q => {
        let e = Z(Q.state).resolveInner(Q.pos, -1);
        if (T.indexOf(e.name) > -1) return null;
        let i = e.name == "VariableName" || e.to - e.from < 20 && S.test(Q.state.sliceDoc(e.from, e.to));
        if (!i && !Q.explicit) return null;
        let a = [];
        for (let t = e; t; t = t.parent) r.has(t.name) && (a = a.concat(x(Q.state.doc, t)));
        return {
            options: a,
            from: i ? e.from : Q.pos,
            validFor: S
        ...
        parser: R.configure({
            props: [ q.add({
                ...
                "SwitchBlock SelectBlock": Q => {
                    ...
                    return Q.baseIndent + (i || a ? 0 : Q.unit);
                },
                Block: v({
                    closing: "}"
                }),
                ...
                "Block SwitchBlock SelectBlock LiteralValue InterfaceType StructType SpecList": D,
                BlockComment: Q => ({
                    from: Q.from + 2,
                    to: Q.to - 2
                })
    ...
    const u = "interface struct chan map package go return break continue goto fallthrough else defer range true false nil".split(" ").map(y);
    s = function() {
        let Q = W.concat(u);
        return new z(l, [ l.data.of({
            autocomplete: F(T, B(Q))
        }), l.data.of({
            autocomplete: p
        }) ]);
    };
