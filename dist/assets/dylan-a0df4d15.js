...
i.simpleDefinition = i.constantSimpleDefinition.concat(i.variableSimpleDefinition).concat(i.otherSimpleDefinition), 
i.keyword = i.statement.concat(i.separator).concat(i.other);

var c = "[-_a-zA-Z?!*@<>$%]+", k = new RegExp("^" + c), l = {
    symbolKeyword: c + ":",
    symbolClass: "<" + c + ">",
    symbolGlobal: "\\*" + c + "\\*",
    symbolConstant: "\\$" + c
...
for (var f in l) l.hasOwnProperty(f) && (l[f] = new RegExp("^" + l[f]));

l.keyword = [ /^with(?:out)?-[-_a-zA-Z?!*@<>$%]+/ ];
...
function u(e, t) {
    var n = e.peek();
    if (n == "'" || n == '"') return e.next(), s(e, t, h(n, "string"));
    if (n == "/") {
        if (e.next(), e.eat("*")) return s(e, t, v);
        if (e.eat("/")) return e.skipToEnd(), "comment";
        e.backUp(1);
    } else if (/[+\-\d\.]/.test(n)) {
        if (e.match(/^[+-]?[0-9]*\.[0-9]*([esdx][+-]?[0-9]+)?/i) || e.match(/^[+-]?[0-9]+([esdx][+-]?[0-9]+)/i) || e.match(/^[+-]?\d+/)) return "number";
    } else {
        if (n == "#") return e.next(), (n = e.peek()) == '"' ? (e.next(), s(e, t, h('"', "string"))) : n == "b" ? (e.next(), 
        e.eatWhile(/[01]/), "number") : n == "x" ? (e.next(), e.eatWhile(/[\da-f]/i), 
        "number") : n == "o" ? (e.next(), e.eatWhile(/[0-7]/), "number") : n == "#" ? (e.next(), 
        "punctuation") : n == "[" || n == "(" ? (e.next(), "bracket") : e.match(/f|t|all-keys|include|key|next|rest/i) ? "atom" : (e.eatWhile(/[-a-zA-Z]/), 
        "error");
        if (n == "~") return e.next(), (n = e.peek()) == "=" ? (e.next(), (n = e.peek()) == "=" && e.next(), 
        "operator") : "operator";
        if (n == ":") {
            if (e.next(), (n = e.peek()) == "=") return e.next(), "operator";
    ...
    for (var o in l) if (l.hasOwnProperty(o)) {
        var r = l[o];
        if (r instanceof Array && y(r, function(a) {
            return e.match(a);
        }) || e.match(r)) return x[o];
    }
    return /[+\-*\/^=<>&|]/.test(n) ? (e.next(), "operator") : e.match("define") ? "def" : (e.eatWhile(/[\w\-]/), 
    p.hasOwnProperty(e.current()) ? b[e.current()] : e.current().match(k) ? "variable" : (e.next(), 
    "variableName.standard"));
}

function v(e, t) {
    for (var n, o = !1, r = !1, a = 0; n = e.next(); ) {
        if (n == "/" && o) {
            if (!(a > 0)) {
                t.tokenize = u;
                break;
...
function h(e, t) {
    return function(n, o) {
        for (var r, a = !1, m = !1; (r = n.next()) != null; ) {
            if (r == e && !a) {
                m = !0;
                break;
            }
        ...
        return !m && a || (o.tokenize = u), t;
    };
}

d([ "keyword", "definition", "simpleDefinition", "signalingCalls" ], function(e) {
    d(i[e], function(t) {
        p[t] = e, b[t] = D[e];
    });
});
