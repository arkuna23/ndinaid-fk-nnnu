function l(a) {
    var c = a.client || {}, g = a.atoms || {
        false: !0,
        true: !0,
        null: !0
    }, p = a.builtin || e(z), C = a.keywords || e(d), _ = a.operatorChars || /^[*+\-%<>!=&|~^\/]/, s = a.support || {}, y = a.hooks || {}, S = a.dateSQL || {
        date: !0,
        time: !0,
        timestamp: !0
    }, Q = a.backslashStringEscapes !== !1, N = a.brackets || /^[\{}\(\)\[\]]/, v = a.punctuation || /^[;.,:]/;
    function h(t, i) {
        var r = t.next();
        if (y[r]) {
            var n = y[r](t, i);
        ...
        if (s.hexNumber && (r == "0" && t.match(/^[xX][0-9a-fA-F]+/) || (r == "x" || r == "X") && t.match(/^'[0-9a-fA-F]*'/)) || s.binaryNumber && ((r == "b" || r == "B") && t.match(/^'[01]+'/) || r == "0" && t.match(/^b[01]*/))) return "number";
        if (r.charCodeAt(0) > 47 && r.charCodeAt(0) < 58) return t.match(/^[0-9]*(\.[0-9]+)?([eE][-+]?[0-9]+)?/), 
        s.decimallessFloat && t.match(/^\.(?!\.)/), "number";
        if (r == "?" && (t.eatSpace() || t.eol() || t.eat(";"))) return "macroName";
        if (r == "'" || r == '"' && s.doubleQuote) return i.tokenize = x(r), i.tokenize(t, i);
        if ((s.nCharCast && (r == "n" || r == "N") || s.charsetCast && r == "_" && t.match(/[a-z][a-z0-9]*/i)) && (t.peek() == "'" || t.peek() == '"')) return "keyword";
        if (s.escapeConstant && (r == "e" || r == "E") && (t.peek() == "'" || t.peek() == '"' && s.doubleQuote)) return i.tokenize = function(m, k) {
            return (k.tokenize = x(m.next(), !0))(m, k);
        }, "keyword";
        if (s.commentSlashSlash && r == "/" && t.eat("/") || s.commentHash && r == "#" || r == "-" && t.eat("-") && (!s.commentSpaceRequired || t.eat(" "))) return t.skipToEnd(), 
        "comment";
        if (r == "/" && t.eat("*")) return i.tokenize = b(1), i.tokenize(t, i);
        if (r != ".") {
            if (_.test(r)) return t.eatWhile(_), "operator";
            if (N.test(r)) return "bracket";
            ...
            return S.hasOwnProperty(o) && (t.match(/^( )+'[^']*'/) || t.match(/^( )+"[^"]*"/)) ? "number" : g.hasOwnProperty(o) ? "atom" : p.hasOwnProperty(o) ? "type" : C.hasOwnProperty(o) ? "keyword" : c.hasOwnProperty(o) ? "builtin" : null;
        }
        return s.zerolessFloat && t.match(/^(?:\d+(?:e[+-]?\d+)?)/i) ? "number" : t.match(/^\.+/) ? null : s.ODBCdotTable && t.match(/^[\w\d_$#]+/) ? "type" : void 0;
    }
    function x(t, i) {
        return function(r, n) {
            for (var o, m = !1; (o = r.next()) != null; ) {
                if (o == t && !m) {
                    n.tokenize = h;
                    break;
                }
    ...
    function b(t) {
        return function(i, r) {
            var n = i.match(/^.*?(\/\*|\*\/)/);
            return n ? n[1] == "/*" ? r.tokenize = b(t + 1) : r.tokenize = t > 1 ? b(t - 1) : h : i.skipToEnd(), 
            "comment";
        };
    }
    function w(t, i, r) {
    ...
    return {
        ...
        token: function(t, i) {
            if (t.sol() && i.context && i.context.align == null && (i.context.align = !1), 
            i.tokenize == h && t.eatSpace()) return null;
            var r = i.tokenize(t, i);
            if (r == "comment") return r;
            i.context && i.context.align == null && (i.context.align = !0);
            var n = t.current();
            return n == "(" ? w(t, i, ")") : n == "[" ? w(t, i, "]") : i.context && i.context.type == n && function(o) {
                o.indent = o.context.indent, o.context = o.context.prev;
            }(i), r;
        },
        indent: function(t, i, r) {
            var n = t.context;
            if (!n) return null;
            var o = i.charAt(0) == n.type;
            return n.align ? n.col + (o ? 0 : 1) : n.indent + (o ? 0 : r.unit);
        },
        languageData: {
            commentTokens: {
...
function u(a) {
    return a.eat("@") && (a.match("session."), a.match("local."), a.match("global.")), 
    a.eat("'") ? (a.match(/^.*'/), "string.special") : a.eat('"') ? (a.match(/^.*"/), 
    "string.special") : a.eat("`") ? (a.match(/^.*`/), "string.special") : a.match(/^[0-9a-zA-Z$\.\_]+/) ? "string.special" : null;
}
...
function e(a) {
    for (var c = {}, g = a.split(" "), p = 0; p < g.length; ++p) c[g[p]] = !0;
    return c;
}
