    ...
    if (r) {
        ...
        if (k.splice) {
            for (var l = 0; l < k.length; ++l) {
                var n = k[l];
                if (n.regex && e.match(n.regex)) return t.next = n.next || t.next, 
                n.token;
            }
            return e.next(), "error";
        }
        if (e.match(n = a[r])) return n.regex && e.match(n.regex) ? (t.next = n.next, 
        n.token) : (e.next(), "error");
    }
    return e.next(), "error";
}, g = "(?![\\d\\s])[$\\w\\xAA-\\uFFDC](?:(?!\\s)[$\\w\\xAA-\\uFFDC]|-[A-Za-z])*", p = RegExp("(?:[({[=:]|[-~]>|\\b(?:e(?:lse|xport)|d(?:o|efault)|t(?:ry|hen)|finally|import(?:\\s*all)?|const|var|let|new|catch(?:\\s*" + g + ")?))\\s*$"), o = "(?![$\\w]|-[A-Za-z]|\\s*:(?![:=]))", x = {
    token: "string",
    regex: ".+"
}, a = {
    start: [ {
...
for (var d in a) {
    var s = a[d];
    if (s.splice) for (var i = 0, u = s.length; i < u; ++i) {
        var c = s[i];
        typeof c.regex == "string" && (a[d][i].regex = new RegExp("^" + c.regex));
    } else typeof c.regex == "string" && (a[d].regex = new RegExp("^" + s.regex));
}
